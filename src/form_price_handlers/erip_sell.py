...
from src.requests.get_request_sell.erip_request import get_offers

router = APIRouter()


@router.post("/erip-offers")
async def add_erip_price(
        request: Request,
        offers_info: dict = Depends(get_offers),
    ...
    if existing_value:
        try:
            new_price_value = update(PriceValueBYN).where(PriceValueBYN.user_id == user_id).values(erip_sell=price_value)
            await session.execute(new_price_value)
            await session.commit()
            return templates_sell.TemplateResponse("erip_request_templates/erip_offers_request.html", {"request": request, "user_info": offers_info})
