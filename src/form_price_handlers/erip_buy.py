...
@router.post("/erip-offers-buy")
async def add_erip_buy_price(
        request: Request,
        offers_info: dict = Depends(get_erip_offers_buy),
    ...
    if existing_value:
        try:
            new_price_value = update(PriceValueBYN).where(PriceValueBYN.user_id == user_id).values(erip_buy=price_value)
