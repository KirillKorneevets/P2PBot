...
@router.post("/card2card-offers-buy")
async def add_card2card_buy_price(
        request: Request,
        offers_info: dict = Depends(get_card2card_offers_buy),
    ...
    if existing_value:
        try:
            new_price_value = update(PriceValueBYN).where(PriceValueBYN.user_id == user_id).values(card2card_buy=price_value)
