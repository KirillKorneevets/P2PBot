...
@router.post("/alfabank-offers")
async def add_alfabank_sell_price(
        request: Request,
        offers_info: dict = Depends(get_alfabank_offers),
    ...
    if existing_value:
        try:
            new_price_value = update(PriceValueBYN).where(PriceValueBYN.user_id == user_id).values(alfabank_sell=price_value)
